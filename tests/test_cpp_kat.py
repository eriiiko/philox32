"""The C++ header, built with MSVC and run against the KAT file, plus the
bit-for-bit cross-language comparison with the Python twin on 64 pairs.

Properties protected: philox32.h IS Philox-4x32-10 (KAT), and philox32.h and
philox32.py are the same function (cross-language identity), including the two
Q16 mappings.
"""
...
def test_cpp_and_python_q16_mappings_agree(cpp_run):
    _, info = cpp_run
    for i, p in info["pairs"].items():
